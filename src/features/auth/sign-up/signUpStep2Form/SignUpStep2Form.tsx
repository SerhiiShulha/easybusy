...
interface ParentSchema extends RegistrationSchema {
  address: {
    addressLine1: string
    addressLine2: string
    city: string
...
interface OrganizerSchema extends RegistrationSchema {
  businessName: string
}
...
const validationSchema = Yup.object({
  ...
  businessName: Yup.string()
    .max(100)
    .when('role', {
      is: (value: string) => value === 'organizer',
...
const SignUpStep2Form: React.FC = () => {
  ...
  const registerOrganizer = async (info: OrganizerSchema) => {
    try {
  ...
  const registerParent = async (info: ParentSchema) => {
    try {
      const response = await fetch(
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const organizerData: OrganizerSchema = { ...data, businessName }
    const parentData: ParentSchema = {
      ...data,
      address: {
        state: state?.value ?? '',
        city: city?.value ?? '',
